...
{
	for (std::map < std::string, SDL_Texture* >::iterator it = _textures.begin(); it !=
		_textures.end(); it++)
		{
		...
		for (std::map < std::string, TTF_Font* >::iterator it = _fonts.begin(); it != _fonts.end();
			it++)
		{
...
void RenderManager::Init()
{
	...
	SDL_SetRenderDrawColor(_renderer, 210, 0, 0, 0xFF);
